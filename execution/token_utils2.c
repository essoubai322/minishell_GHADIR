...
{
	global.stripped_value = get_stripped_value(type, value,
			global.stripped_value);
	if (type == WORD)
	{
		if (value && strchr(value, '$') != 0)
		{
			...
			while (value[global.o] != '\0')
			{
				if (value[global.o] == '$')
				{
					...
					add_token_env_value(global.env_value,
						&global.stripped_value, value, &global.o);
					if (value[global.o] != '$' && ft_strchr(value + global.o,
							'$') == 0)
						if (add_token_check(head, global.stripped_value, type))
							return ;
				}
				else
					add_token_else(value, &global.o, &global.stripped_value,
						&global.k);
			}
		}
		else
